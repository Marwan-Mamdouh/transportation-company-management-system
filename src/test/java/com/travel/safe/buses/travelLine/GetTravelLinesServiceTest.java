...
class GetTravelLinesServiceTest {
  ...
  void givenTravelLineExist_whenGetTravelLinesCalled_returnTravelLineList() {
    ...
    final var travelLineDTOs = travelLines.stream().map(mapper::toResponse).toList();

    // when
    when(travelLineRepository.findAll()).thenReturn(travelLines);
    final var response = getTravelLinesService.execute(null);

    // then
    assertEquals(travelLineDTOs, response);
